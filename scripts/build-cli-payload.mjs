...
function assertArchivePrefix(archivePrefix) {
  if (
    typeof archivePrefix !== "string" ||
    !/^[A-Za-z0-9][A-Za-z0-9._-]*\/$/u.test(archivePrefix)
  ) {
    throw new CliPayloadError("PAYLOAD_PREFIX_UNSAFE");
  }
}

export function createCliPayloadArchive({
  archivePrefix,
  manifestPath,
  repositoryRoot,
  treeish = "HEAD",
}) {
  assertArchivePrefix(archivePrefix);
  ...
  // reviewed tree; nothing is copied through a working directory. The exact
  // prefix is part of the contract because the core CLI verifies it before
  // extracting any bytes.
  const artifact = execFileSync(
    "git",
    ["archive", "--format=tar", `--prefix=${archivePrefix}`, treeish, "--", ...members],
...
export function buildCliPayload({
  ...
  const { artifact, members } = createCliPayloadArchive({
    archivePrefix,
    manifestPath,
    repositoryRoot,
    treeish,
  });
  writeFileSync(outputPath, artifact, { flag: "wx" });

  return {
    integrity: `sha256-${createHash("sha256").update(artifact).digest("base64")}`,
...
function isDirectExecution() {
  // pathToFileURL, not string concatenation: a Windows path is not a valid
  // file URL by prefixing "file://".
  return Boolean(
...
if (isDirectExecution()) {
  ...
  const templateRelease = JSON.parse(
    readFileSync("template/mobile-template.manifest.json", "utf8"),
  ).templateRelease;
